...
export function isWebp() {
	...
	// Добавление класса _webp або _no-webp для HTML
...
export function showMore() {
	window.addEventListener("load", function (e) {
		...
		function initItem(showMoreBlock, matchMedia = false) {
			...
			const hiddenHeight = getHeight(showMoreBlock, showMoreContent) + 10;
			if (matchMedia.matches || !matchMedia) {
		...
		function getHeight(showMoreBlock, showMoreContent) {
			...
			if (showMoreType === 'items') {
				...
				for (let index = 1; index < showMoreItems.length; index++) {
					...
					const marginTop = parseFloat(getComputedStyle(showMoreItem).marginTop) ? parseFloat(getComputedStyle(showMoreItem).marginTop) : -20;
...
export function customCursor(isShadowTrue) {
	...
	if (wrapper && !isMobile.any()) {
		// Ссоздаем и добавляем объект курсора
