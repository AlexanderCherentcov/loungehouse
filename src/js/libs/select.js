...
Data-show для плейсхолдера, показывает его в списке (только для одиночного выбора)
...
class SelectConstructor {
	constructor(props, data = null) {
		...
		if (this.config.init) {
			...
			if (selectItems.length) {
				...
				this.setLogging(`Проснулся, построил селект: (${selectItems.length})`);
			} else {
	...
	// Функция инициализации всех селектив
	...
	// Функция закрытия всех селектив
	...
	selectAction(selectItem) {
		const originalSelect = this.getSelectElement(selectItem).originalSelect;
		const selectOptions = this.getSelectElement(selectItem, this.selectClasses.classSelectOptions).selectElement;
		const selectOpenzIndex = originalSelect.dataset.zIndex ? originalSelect.dataset.zIndex : 5;
	...
	getOption(selectOption, originalSelect) {
		// Если элемент выбран и включен режим мульти выбора, добавляем класс
	...
	optionAction(selectItem, originalSelect, optionItem) {
		...
		if (!selectOptions.classList.contains('_slide')) {
			if (originalSelect.multiple) { // Если мульти выбор
