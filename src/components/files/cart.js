document.addEventListener('DOMContentLoaded', () => {
	...
	const telSelectorOrder = formOrder.querySelector('input[type="tel"]');
		...
		new window.JustValidate(".order", {
			rules: {
				tel: {
					required: true,
					function: () => {
						const phone = telSelectorOrder.inputmask.unmaskedvalue();
					...
					xhr.onreadystatechange = function() {
						...
						if (xhr.status === 200) {
							...
							tl.fromTo(
								".sent-popup",
								{ autoAlpha: 1 },
								{ duration: 3, ease: "power4.out", autoAlpha: 0 },
								2 );
							} else {
								let tl = gsap.timeline();
								tl.fromTo(
